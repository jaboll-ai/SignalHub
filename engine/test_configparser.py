...
class TestConfigParser(unittest.TestCase):
    ...
    def test_start_with_missing_file(self, mock_exists, mock_open):
        ...
        config_parser = ConfigParser(
            argumentParser=MockedArgumentParser(argparse.Namespace(cfg="other.yml"))
        )
    ...
    @patch("os.path.exists", return_value=True)
    @patch("yaml.safe_load")
    def test_command_line_argument_overwrite(
        self, mock_yaml_load, mock_exists, mock_open
    ):
        # Mock YAML content
        mock_yaml_load.return_value = {"key1": "value1", "key2": "value2"}

        config_parser = ConfigParser(
            argumentParser=MockedArgumentParser(
                argparse.Namespace(cfg="config.yml", key1="new_value")
            )
        )
        config_parser.start(None)
